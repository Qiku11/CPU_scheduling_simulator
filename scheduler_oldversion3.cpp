...
const int TIME_ALLOWANCE = 3;  // allow to use up to this number of time slots at once
const int AGING_LIMIT = 20;
    ...
    int main(int argc, char *argv[])
    {
      ...
      for (int current_time = 0; !all_done; current_time++)
      {
        ...
        if (current_id == -1)
        {
          ...
          // priority_queue<pair<int, int>, vector<pair<int, int>>, greater<pair<int, int>>> customer_queue_helper = queue_low;
          // while(!customer_queue_helper.empty()){
          //   if (customers[customer_queue_helper.top().second].waiting_since > AGING_LIMIT){
          //
          //   }
